...
export default async function p2pRoutes(
  ...
  // TODO Below are on the chopping block, for the demo we're only likely to use the all-in-one handler above
  server.get("/p2p/transmissionHistory", {
    handler: (request, reply) => {
      return reply.status(200).send(transmissionHistory);
    }
  })
  server.get("/p2p/eventHistory", {
    handler: (request, reply) => {
      return reply.status(200).send(eventHistory);
    }
  })

  server.get<{Querystring: SubscibersQuerystringType}>("/p2p/subscribers", {
    schema: {
      querystring: SubscibersQuerystring,
    },
    handler: async (request, reply) => {
      const {topic} = request.query
      const topics = libp2pNode.services.pubsub.getSubscribers(topic)
      return reply.status(200).send(topics);
    },
  });

  server.get("/p2p/listenAddresses", {
    handler: (request, reply) => {
      return reply.status(200).send(libp2pNode.getMultiaddrs());
    }
  })
  server.get("/p2p/peerId", {
    handler: (request, reply) => {
      return reply.status(200).send(libp2pNode.peerId);
    }
  })
  server.get("/p2p/topics", {
    schema: {},
    handler: async (request, reply) => {
      const topics = libp2pNode.services.pubsub.getTopics()
      return reply.status(200).send(topics);
  ...
  server.get("/p2p/peers", {
    schema: {},
    handler: async (request, reply) => {
      const peers = libp2pNode.services.pubsub.getPeers()
